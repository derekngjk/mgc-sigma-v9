...
def test_generate_visual_calls_imagen(monkeypatch):
    fake_png = b"\x89PNG\r\n\x1a\nfakedata"
    ...
    with patch("google.genai.Client", return_value=fake_client):
        result = generate_visual(["breast cancer"], "Your care summary explains the treatment plan.")

    assert result == fake_png
    call_kwargs = fake_client.models.generate_images.call_args
    assert call_kwargs.kwargs["model"] == "imagen-3.0-generate-002"
    assert "breast cancer" in call_kwargs.kwargs["prompt"]
    assert "treatment plan" in call_kwargs.kwargs["prompt"]


def test_generate_visual_falls_back_to_nano_banana(monkeypatch):
    import base64
    ...
    fake_client = MagicMock()
    fake_client.models.generate_images.side_effect = RuntimeError("quota exceeded")
...
def test_generate_visual_missing_key_raises(monkeypatch):
    monkeypatch.delenv("GEMINI_API_KEY", raising=False)
    with pytest.raises(ImageConfigError, match="GEMINI_API_KEY not set"):
        generate_visual(["some condition"])


def test_generate_visual_both_apis_fail_raises(monkeypatch):
    monkeypatch.setenv("GEMINI_API_KEY", "test-key")
...
def test_approve_calls_image_generation_when_flag_true(client, mock_supabase):
    ...
    mock_fn.assert_called_once_with("comm-001", ["invasive ductal carcinoma"], "Your care summary.")
